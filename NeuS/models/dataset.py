...
class Dataset:
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        device = self.intrinsics_all_inv.device
        ...
        #    注意：高级索引要求「被索引的张量」和「索引」在同一个 device
        img = self.images[img_idx]  # 可能在 cpu
        msk = self.masks[img_idx]  # 可能在 cpu

        # 把索引临时搬到 img 所在设备进行索引
        px_img = pixels_x.to(img.device)
        py_img = pixels_y.to(img.device)

        color = img[(py_img, px_img)]  # batch_size, 3   (在 img.device)
        mask = msk[(py_img, px_img)]  # batch_size, 3   (在 img.device)

        # 然后再把 color / mask 搬回统一 device
        ...
        p = torch.matmul(
            self.intrinsics_all_inv[img_idx, None, :3, :3].to(device),  # 1,3,3
            p[:, :, None],  # B,3,1
        ).squeeze(
            -1
        )  # batch_size, 3  （只去掉最后一个维度）

        # 4. 得到射线方向（世界坐标）
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)  # B,3

        rays_v = torch.matmul(
            self.pose_all[img_idx, None, :3, :3].to(device),  # 1,3,3
            rays_v[:, :, None],  # B,3,1
        ).squeeze(
            -1
        )  # B,3
        ...
        # 5. 射线起点（世界坐标）
        rays_o = self.pose_all[img_idx, None, :3, 3].to(device).expand_as(rays_v)  # B,3

        # 6. 最终拼接，全部在同一个 device 上，不再来回 .cpu() / .cuda()
        return torch.cat([rays_o, rays_v, color, mask[:, :1]], dim=-1)  # B, 10
